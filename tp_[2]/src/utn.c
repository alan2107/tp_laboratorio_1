...
void pW2SubMenu2(Passenger *list1, Flight *list2, int lenght, int index) {
	if(list1 != NULL && list2 != NULL && lenght > 0 && index >= 0) {
		printf("\n***********************/MENU PRINCIPAL > SUBMENÚ MODIFICACIÓN DE PASAJERO\n");
		printf("\n1. MODIFICAR NOMBRE: %s", list1[index].name);
		printf("\n2. MODIFICAR APELLIDO: %s", list1[index].lastName);
		printf("\n3. MODIFICAR TIPO: ");
		...
		printf("\n4. MODIFICAR PRECIO DE VUELO: $%.2f", list1[index].price);
		printf("\n5. MODIFICCAR CODIGO DE VUELO: %s", list1[index].flyCode);
